...
impl StreamProducer {
    ...
    pub fn remove_consumer(&self, consumer_id: &str) {
        if self
            ...
            .is_some()
        {
            debug!(appsink = self.appsink().name().as_str(), consumer_id = consumer_id, "Removed consumer");
        } else {
            debug!(appsink = self.appsink().name().as_str(), consumer_id = consumer_id, "Consumer not found");
        }
    }
    ...
    // }
    // assuming producer is your StreamProducer instance
    // producer.set_forward_events(vec![gst::EventType::Caps, gst::EventType::Tag]);
    // To set events_to_forward to not include the EOS events, you just need to 
    // call the set_forward_events function with the list of events you want to 
    // forward that doesn't include gst::EventType::Eos.

    // Here's an example of how you can do it:
    
    // // assuming producer is your StreamProducer instance
    // producer.set_forward_events(vec![gst::EventType::Caps, gst::EventType::Tag]);
    // This will set the events_to_forward to only include Caps and Tag events,
    //  and not Eos events. 

    // Note that the set_forward_events function takes an impl IntoIterator<Item = gst::EventType>,
    //  so you can pass in any iterable of gst::EventType, such as a 
    //  Vec, HashSet, etc.  

    pub fn forward(&self) {
        // get all event types except Eos
        let events_to_forward = vec![];
    
        // set these events to be forwarded
        self.inner.lock().unwrap().producer.set_forward_events(events_to_forward);
    }
